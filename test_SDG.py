...
def plot_decision_boundary(X, T):
    ...
    V1 = tanh(-t1 + w1 @ grid.T)
    V2 = tanh(-t2 + w2 @ V1)
    O = tanh(-t3 + (w3.T @ V2)).reshape(-1)
    Z = O.reshape(xx.shape)

    plt.figure(figsize=(8, 6))
    plt.contourf(xx, yy, Z, levels=np.linspace(-1, 1, 50), cmap="coolwarm", alpha=0.5)
    plt.contour(xx, yy, Z, levels=[0], colors='black', linewidths=1.5)
    plt.scatter(X[:, 0], X[:, 1], c=T, cmap="coolwarm", edgecolors='none', alpha=0.8, s=20)
    plt.title("Decision Boundary (Enhanced Visibility)")
